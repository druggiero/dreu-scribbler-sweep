...
def decodeLines(pic, blb, avX): #uses a line of sight (avX), a color picture to read the binary (pic), a b&w picture differentiate between the lines and the sign (bw) (used to include signColor but it was never used so it was deleted as a parameter.) Returns the binary interpretation of the sign in a list. It reads top to bottom.
    ...
    blbMark = makeNewPhotoCopy(blb) #debug
    y=0
...
def findBlobs(pic, blbAmt): #finds objects of interest in a black and white photo. Inputs: black and white photo. Outputs: list [x1.1, y1.1, x1.2, y1.2, x2.1, y2.1, x2.2 ...] where xa.1, ya.1 defines the top left corner of rectangle of interest a, and xa.2, ya.2 is the lower right corner. Dependecies: normalize, group, squarify. 
    ...
    #wait(1)
    timer = default_timer() #debug 
    mypic = normalize(pic, blbAmt)#CHANGER 5 7 9
    show(mypic)
...
def findSigns(blbAmt):
    ...
    show(pic) #debug
    #wait(1) #debug
    print("findSigns: converting to bw")
    bw = bwConverter(pic, "new pink") #new pink
    print("findSigns: done") #took 19.09 seconds (20 seconds)
    show(bw) #debug
    #wait(1) #debug
    #print("findRobots: time was", (default_timer()-start)) #debug
    print("findSigns: finding blobs")
    #start_two = default_timer() #debug
    coords = findBlobs(bw, blbAmt)
    print("findSigns: done") #took 2:09.31 minutes (2 minutes and 10 seconds)
    #print("findRobots: time was", (default_timer()-start_two)) #debug
    print("findSigns: here is coords ", coords)
    lookBoxes = [findBiggestBoxes(coords, "index")]
    ...
    if len(lookBoxes) > 1: #if there are more than one biggest box
        ...
        for i in lookBoxes:
            ...
            center = findBoxCenter(box)
            dist = pointDistance(center, [getWidth(bw)/2, getHeight(bw)/2])
            if  closest == "none" or dist < closest:
                closest = dist
                closestIndex = i
        lookBoxes = [closestIndex]
    print("findSigns: These are the boxes I'm looking at:", lookBoxes)
    print("findSigns: crossing out and reading boxes")
    bwBlob = normalize(bw, blbAmt)
    theBox = boxAtIndex(lookBoxes[0], coords)
    signsRead = [blockoutAndRead(coords, lookBoxes, bwBlob, pic), findBoxCenter(theBox), calcBoxSize(theBox)]
    print("findSigns: done:", signsRead) #took 3:14.13 minutes (3 minutes and 15 seconds)
    print("findSigns: time was", (default_timer()-start)) #debug
...
def findBiggestBoxes(coords, returnVal): #returnVal is a string. If it's "box", it returns a list of box coordinates (of the largest box), if it's not box, it returns the index fo the largest box.
